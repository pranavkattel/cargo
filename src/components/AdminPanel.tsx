...
export const AdminPanel: React.FC<AdminPanelProps> = () => {
  ...
  const handleBulkOperation = async (operation: 'update-status' | 'delete', updateData?: any) => {
    ...
    try {
      ...
      // Refresh shipments
      const updatedShipments = await trackingAPI.getAllShipments();
      setShipments(updatedShipments.slice(0, 10));
  ...
  const handleStatusUpdate = async (shipmentId: string, newStatus: string) => {
    try {
      await trackingAPI.updateShipmentStatus(shipmentId, newStatus);
              ...
              <div className="form-section">
                ...
                <div className="form-grid">
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData({
                      ...formData,
                      shipmentDetails: { ...formData.shipmentDetails, serviceType: e.target.value }
                    })}
                    required
                  >
                    <option value="">Select Service Type</option>
                    {dropdownOptions.serviceType?.map(service => (
                      <option key={service.value} value={service.value}>
                        {service.icon} {service.label} - {service.price} ({service.description})
                      </option>
                    ))}
                  </select>
                </div>
